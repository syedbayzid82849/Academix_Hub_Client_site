...
export function Membership() {
    ...
    const handleCheckout = async (plan) => {
        if (plan.isFree) {
            toast.success("🎉 Already selected this free plan!");
            return;
        }
            ...
            <div className="max-w-6xl mx-auto px-6 text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
                    {plans.map((plan, index) => (
                        <div key={index} className={`relative flex flex-col justify-between h-full rounded-2xl p-8 border transition-all duration-300 ${plan.highlight ? "bg-gradient-to-b from-blue-600 to-indigo-700 border-none" : "bg-[#111827] border border-gray-700"}`}>
                            {plan.highlight && <span className="absolute top-0 right-0 mt-2 mr-6 bg-yellow-400 text-black text-sm font-semibold px-3 py-1 rounded-full">Popular</span>}
                            <div>
